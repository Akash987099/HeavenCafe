        ...
        <div class="card mb-4">
            <div class="card-header pb-0 d-flex justify-content-between align-items-center gap-3 flex-wrap">
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    <button type="submit" form="bulkDeliveryForm" id="bulkDeliverButton" class="btn btn-success btn-sm mb-0 opacity-50" disabled onclick="return confirm('Mark all selected orders as delivered? Their items will be added to the respective store stock.');">Mark Selected Delivered</button>
            ...
            @if(session('success'))<div class="mx-3 mt-3 alert alert-success text-sm mb-0">{{ session('success') }}</div>@endif
            @if(session('error'))<div class="mx-3 mt-3 alert alert-danger text-sm mb-0">{{ session('error') }}</div>@endif
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    table.querySelectorAll('tbody tr').forEach(function (row) {
        ...
        if (!isDelivered && orderId) {
            ...
            deliverButton.className = 'btn btn-sm btn-success mb-0 ms-1';
            deliverButton.textContent = 'Deliver';
            deliverButton.addEventListener('click', function () {
                if (!confirm('Mark this order as delivered?')) return;
                const form = document.createElement('form');
                form.method = 'POST';
                form.action = viewLink.href + '/status';
                form.innerHTML = '<input type="hidden" name="_token" value="{{ csrf_token() }}"><input type="hidden" name="status" value="2">';
                document.body.appendChild(form);
                form.submit();
            });
            viewLink.parentElement.appendChild(deliverButton);
        }
