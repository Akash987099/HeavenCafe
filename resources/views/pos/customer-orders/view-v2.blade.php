                    ...
                    @foreach($order->items as $item)
                        <div class="flex items-center justify-between gap-4 px-5 py-4">
                            <div>
                                ...
                                <p class="mt-1 text-xs text-slate-400">₹{{ number_format($item->price, 2) }} × {{ $item->quantity }}</p>
                            </div>
                            <strong class="text-orange-700">₹{{ number_format($item->total, 2) }}</strong>
                        </div>
                    @endforeach
                </div>
            ...
            <aside class="space-y-5">
                <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    @if($order->customer_mobile)
                        <p class="mt-1 text-sm text-slate-500">{{ $order->customer_mobile }}</p>
                    @endif
                    @if($order->customer_email)
                        <p class="mt-1 break-all text-sm text-slate-500">{{ $order->customer_email }}</p>
                    @endif
                </section>

                <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <h2 class="font-bold text-slate-800">Status</h2>
                    <div class="mt-3 flex justify-between text-sm"><span class="text-slate-500">Payment</span><span class="rounded-full bg-amber-50 px-2.5 py-1 font-semibold text-amber-700">{{ ucfirst($order->payment_status ?? 'pending') }}</span></div>
                    <div class="mt-3 flex justify-between text-sm"><span class="text-slate-500">Order type</span><span class="rounded-full bg-blue-50 px-2.5 py-1 font-semibold text-blue-700">{{ ($order->fulfillment_type ?? 'packing') === 'dine_in' ? 'Dine In' : 'Packing' }}</span></div>
                    <div class="mt-3 flex justify-between text-sm"><span class="text-slate-500">Order</span><span class="rounded-full {{ strtolower($order->status) === 'delivered' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600' }} px-2.5 py-1 font-semibold">{{ ucfirst($order->status) }}</span></div>
                    @if(strtolower($order->status) !== 'delivered')
                        <form method="POST" action="{{ route('pos.customer-orders.delivered', $order) }}" class="mt-5">
                            @csrf
                            <button onclick="return confirm('Mark this customer order as delivered?')" class="w-full rounded-xl bg-emerald-600 px-4 py-3 text-sm font-semibold text-white hover:bg-emerald-700">Mark Delivered</button>
                        </form>
                    @endif
                </section>
            </aside>
